...
struct method_get_info g_get_info_data;
...
cJSON *get_info_handler(mjrpc_ctx_t *ctx, cJSON *params, cJSON *id)
{
    ...
    sprintf(temp_str, "%.02f", g_get_info_data.yaw);
    cJSON_AddStringToObject(cjson_info, "Yaw", temp_str);

    sprintf(temp_str, "%.02f", g_get_info_data.depth);
    cJSON_AddStringToObject(cjson_info, "Depth", temp_str);

    sprintf(temp_str, "%.02f", g_get_info_data.temp);
